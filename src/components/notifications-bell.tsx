      ...
      {open && (
        <div className="absolute right-0 mt-2 w-80 bg-industrial border border-white/10 rounded-md shadow-xl z-50">
          <div className="flex items-center justify-between px-3 py-2 border-b border-white/10">
            ...
            {unread > 0 && (
              <button onClick={markAll} className="text-xs text-brand hover:brightness-125 inline-flex items-center gap-1">
          ...
          <div className="max-h-96 overflow-y-auto">
            {items.length === 0 ? (
              ...
              items.map((n) => (
                <div key={n.id} className={`px-3 py-2 border-b border-white/5 flex gap-2 ${!n.read_at ? "bg-brand/5" : ""}`}>
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-semibold truncate">{n.title}</div>
                    {n.body && <div className="text-xs text-slate-400 line-clamp-2">{n.body}</div>}
                    <div className="text-[10px] text-slate-500 font-mono mt-0.5">{new Date(n.created_at).toLocaleString("pt-BR")}</div>
                  </div>
                  <div className="flex flex-col gap-1 shrink-0">
                    {!n.read_at && (
                      <button onClick={() => markRead(n.id)} className="p-1 hover:bg-white/10 rounded" title="Marcar como lida">
                        <Check className="w-3 h-3" />
                      </button>
                    )}
                    <button onClick={() => remove(n.id)} className="p-1 hover:bg-white/10 rounded text-slate-500" title="Remover">
